...
const AdminServiceCard = ({ service, t, locale }) => {
        ...
        {Object.entries(groups).map(([category, svcs], gi) => (
          <div key={category} className="mb-10 last:mb-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5">
              {svcs.map((svc, i) => (
                <Reveal key={svc.id} delay={gi * 60 + i * 60} dir="up" dur={600}>
                  <AdminServiceCard service={svc} t={t} locale={locale} />
                </Reveal>
              ))}
            </div>
          </div>
      ...
      <div className="min-h-screen bg-white overflow-x-hidden">
        ...
        <section className="relative min-h-[55vh] sm:min-h-[65vh] flex items-end justify-center overflow-hidden">
          ...
          <div className="relative z-10 text-center px-4 sm:px-6 pb-12 sm:pb-20 max-w-5xl mx-auto w-full">
            <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20 mb-5 sm:mb-7" style={{ animation: "fadeUp 0.8s 0.1s both ease-out" }}>
              <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse flex-shrink-0" />
              <span className="text-[11px] text-white/65 font-bold uppercase tracking-[0.14em] sm:tracking-[0.18em]">{t("Our Services", "Nuestros Servicios")}</span>
            </div>
            <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-white leading-[1.08] mb-3 sm:mb-4 tracking-tight" style={{ animation: "fadeUp 0.9s 0.25s both ease-out" }}>
              {t("A clean space for", "Un espacio limpio para")}
              <span className="block font-bold">{t("everyone.", "todos.")}</span>
            </h1>
            <p className="text-sm sm:text-lg md:text-xl text-white/65 max-w-xl mx-auto font-medium" style={{ animation: "fadeUp 0.9s 0.4s both ease-out" }}>
          ...
          <div className="absolute bottom-0 left-0 right-0 z-20">
            <svg viewBox="0 0 1440 90" preserveAspectRatio="none" className="w-full h-7 sm:h-14 lg:h-20"><path d="M0,45 C300,0 600,90 1440,45 L1440,90 L0,90 Z" fill="white" /></svg>
          </div>
        </section>
                    ...
                    <div className="relative bg-white rounded-2xl p-5 sm:p-8 border border-slate-100 shadow-lg overflow-hidden group hover:-translate-y-0.5 hover:shadow-xl hover:border-primary/20 transition-all duration-300">
                      ...
                      <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div>
                          <div className="flex items-center gap-2.5 mb-2">
                            ...
                            <h3 className="text-lg sm:text-2xl font-bold text-slate-900 group-hover:text-primary transition-colors duration-200">{t("Self-Service Laundry", "Lavandería de Autoservicio")}</h3>
                          </div>
                          <p className="text-slate-500 text-sm pl-[50px] sm:pl-[52px]">{t("Modern machines, fast dryers, hassle-free experience.", "Máquinas modernas, secadoras rápidas, sin complicaciones.")}</p>
                        </div>
                        <div className="flex gap-5 sm:gap-8 flex-shrink-0 pl-[50px] sm:pl-0">
                          {[{ val: "6:00 AM", label: t("Open", "Abrimos") }, { val: "10:00 PM", label: t("Close", "Cerramos") }].map((h, i) => (
                            <div key={i} className="text-center"><div className="text-lg sm:text-2xl font-black text-primary">{h.val}</div><div className="text-[11px] text-slate-400 uppercase tracking-wider font-semibold">{h.label}</div></div>
                          ))}
                        </div>
                      </div>
                      <div className="relative grid grid-cols-1 sm:grid-cols-3 gap-2 mt-4 pt-4 border-t border-slate-100">
                        {[t("Clean & well-maintained", "Limpio y bien mantenido"), t("High-performance washers", "Lavadoras de alto rendimiento"), t("Fast-drying machines", "Secadoras rápidas")].map((item, i) => (
                          <div key={i} className="flex items-center gap-2 text-sm text-slate-600"><Check className="h-4 w-4 text-sky-400 flex-shrink-0" />{item}</div>
                        ))}
                      </div>
                    </div>
                  </Tilt>
