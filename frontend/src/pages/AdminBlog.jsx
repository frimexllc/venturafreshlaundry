...
import { Plus, Edit2, Trash2, Search, Eye, EyeOff, X, Tag } from "lucide-react";
...
export default function AdminBlog() {
  ...
  const [showImageModal, setShowImageModal] = useState(false); // Modal para seleccionar imagen
  ...
  const [formData, setFormData] = useState({
    ...
    is_published: false,
    featured_image: '' // Nuevo campo para imagen destacada
  });
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (!postsRes.ok) throw new Error('Error fetching posts');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        featured_image: formData.featured_image || DEFAULT_POST_IMAGE // Usar imagen por defecto si no se especifica
      };
      ...
      if (res.ok) {
        toast.success(editingPost ? t('Post updated', 'Post actualizado') : t('Post created', 'Post creado'));
        setShowModal(false);
        resetForm();
        loadData();
      } else {
        const error = await res.json();
        toast.error(error.detail || t('Error saving post', 'Error al guardar post'));
      }
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    if (!confirm(t('Delete this post?', '¿Eliminar este post?'))) return;
    try {
      const res = await fetch(`${API_URL}/api/blog/posts/${id}`, { method: 'DELETE' });
      if (res.ok) {
        toast.success(t('Post deleted', 'Post eliminado'));
        loadData();
      }
    } catch (error) {
  ...
  const togglePublished = async (post) => {
    try {
      ...
      if (res.ok) {
        toast.success(post.is_published ? t('Post unpublished', 'Post despublicado') : t('Post published', 'Post publicado'));
        loadData();
      }
    } catch (error) {
  ...
  const handleCreateCategory = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(t('Category created', 'Categoría creada'));
        setShowCategoryModal(false);
        setCategoryForm({ name: '', description: '' });
        loadData();
      } else {
        const error = await res.json();
        toast.error(error.detail || t('Error creating category', 'Error al crear categoría'));
      }
    } catch (error) {
      toast.error(t('Connection error', 'Error de conexión'));
    }
  ...
  const handleDeleteCategory = async (slug) => {
    if (!confirm(t('Delete this category? Posts will be moved to "general"', '¿Eliminar esta categoría? Los posts se moverán a "general"'))) return;
    try {
      const res = await fetch(`${API_URL}/api/blog/categories/${slug}`, { method: 'DELETE' });
      if (res.ok) {
        toast.success(t('Category deleted', 'Categoría eliminada'));
        loadData();
      }
    } catch (error) {
      toast.error(t('Error deleting', 'Error al eliminar'));
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{t('Blog Management', 'Gestión de Blog')}</h1>
          <p className="text-slate-600">{t('Manage blog posts and categories', 'Administra posts y categorías del blog')}</p>
        </div>
        <div className="flex gap-2">
          {activeTab === 'posts' && (
            <Button onClick={() => { resetForm(); setShowModal(true); }} className="bg-sky-600 hover:bg-sky-700" data-testid="add-post-btn">
              <Plus className="h-4 w-4 mr-2" /> {t('New Post', 'Nuevo Post')}
            </Button>
          )}
          {activeTab === 'categories' && (
            <Button onClick={() => setShowCategoryModal(true)} className="bg-sky-600 hover:bg-sky-700" data-testid="add-category-btn">
              <Plus className="h-4 w-4 mr-2" /> {t('New Category', 'Nueva Categoría')}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="flex gap-2 border-b border-slate-200">
        ...
        >
          <img 
            ...
            alt="Posts" 
            className="h-5 w-5 inline mr-2 rounded-full object-cover"
        ...
        >
          ...
          {t('Categories', 'Categorías')} ({categories.length})
        </button>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="relative max-w-md">
            ...
            <Input
              placeholder={t('Search posts...', 'Buscar posts...')}
              value={searchQuery}
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">{t('Image', 'Imagen')}</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">{t('Title', 'Título')}</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">{t('Category', 'Categoría')}</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">{t('Author', 'Autor')}</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">{t('Views', 'Vistas')}</th>
                    ...
                    <tr key={post.id} data-testid={`admin-post-${post.id}`}>
                      ...
                      <td className="px-4 py-3">
                        ...
                        >
                          ...
                          {post.is_published ? t('Published', 'Publicado') : t('Draft', 'Borrador')}
                        </button>
                      </td>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {categories.map((cat) => (
            <div key={cat.slug} className="bg-white rounded-xl border border-slate-200 p-4" data-testid={`admin-category-${cat.slug}`}>
              ...
              <p className="text-sm text-slate-600 mb-2">{cat.description || t('No description', 'Sin descripción')}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4 p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold">{editingPost ? t('Edit Post', 'Editar Post') : t('New Post', 'Nuevo Post')}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <Label htmlFor="featured_image">{t('Featured Image URL', 'URL de Imagen Destacada')}</Label>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  {t('Leave empty to use default image', 'Deja vacío para usar imagen por defecto')}
                </p>
              </div>
              
              <div>
                <Label htmlFor="title">{t('Title', 'Título')}</Label>
                <Input
                  id="title"
              ...
              <div>
                <Label htmlFor="excerpt">{t('Excerpt', 'Extracto')}</Label>
                <textarea
                  id="excerpt"
                  ...
                  rows={2}
                  placeholder={t('Brief description of the post...', 'Breve descripción del post...')}
                />
              </div>
              <div>
                <Label htmlFor="content">{t('Content (HTML)', 'Contenido (HTML)')}</Label>
                <textarea
                  id="content"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="author">{t('Author', 'Autor')}</Label>
                  <Input
                    id="author"
              ...
              <div>
                <Label htmlFor="tags">{t('Tags (comma separated)', 'Tags (separados por coma)')}</Label>
                <Input
                  id="tags"
                  value={formData.tags}
                  onChange={(e) => setFormData({ ...formData, tags: e.target.value })}
                  placeholder={t('laundry, tips, advice', 'lavandería, consejos, tips')}
                />
              </div>
              <div className="flex items-center gap-2">
                ...
                />
                <Label htmlFor="is_published" className="cursor-pointer">{t('Publish immediately', 'Publicar inmediatamente')}</Label>
              </div>
              <div className="flex gap-2 pt-4">
                ...
                <Button type="submit" className="flex-1 bg-sky-600 hover:bg-sky-700">
                  {editingPost ? t('Update', 'Actualizar') : t('Create', 'Crear')}
                </Button>
              </div>
            </form>
          </div>
      ...
      {showCategoryModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold">{t('New Category', 'Nueva Categoría')}</h2>
            ...
            <form onSubmit={handleCreateCategory} className="space-y-4">
              <div>
                <Label htmlFor="cat_name">{t('Name', 'Nombre')}</Label>
                <Input
                  id="cat_name"
              ...
              <div>
                <Label htmlFor="cat_desc">{t('Description', 'Descripción')}</Label>
                <textarea
                  id="cat_desc"
