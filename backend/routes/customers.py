...
async def _check_membership_active(customer_id: str):
    """Check if customer has an active membership"""
    cust = await db.customers.find_one({"id": customer_id}, {"_id": 0, "membership_status": 1})
    if not cust or cust.get("membership_status") != "active":
        raise HTTPException(status_code=403, detail="Active membership required to manage preferences")


@router.get("/customer/membership-status")
async def get_customer_membership_status(current_customer: dict = Depends(get_current_customer)):
    """Check if the logged-in customer has an active membership"""
    cust = await db.customers.find_one({"id": current_customer["id"]}, {"_id": 0, "membership_status": 1, "membership_plan": 1})
    return {
        "has_membership": cust.get("membership_status") == "active" if cust else False,
        "membership_plan": cust.get("membership_plan") if cust else None,
        "membership_status": cust.get("membership_status") if cust else None,
    }


@router.get("/customer/pending-payments")
async def get_customer_pending_payments(current_customer: dict = Depends(get_current_customer)):
    """Get unpaid orders for the logged-in customer"""
    orders = await db.orders.find(
        {"customer_id": current_customer["id"], "payment_status": {"$ne": "paid"}},
        {"_id": 0, "id": 1, "order_number": 1, "service_type": 1, "status": 1,
         "payment_status": 1, "total_amount": 1, "subtotal": 1, "delivery_fee": 1,
         "processing_fee": 1, "actual_lbs": 1, "created_at": 1},
    ).sort("created_at", -1).to_list(50)
    return orders


@router.get("/customer/preferences", response_model=PreferenceResponse)
async def get_current_customer_preferences(current_customer: dict = Depends(get_current_customer)):
    await _check_membership_active(current_customer["id"])
...
@router.post("/customer/preferences", response_model=PreferenceResponse)
...
@router.delete("/customer/preferences")
