...
CORRECCIONES:
- Links de pago: ahora se genera un enlace interno del CRM (https://dominio/account?pay_order=...)
  que redirige al checkout real.
- Eliminada función _voice duplicada (la última versión no tenía entrada "processing").
- Mensaje de voz personalizado para "processing" (ropa recogida y en proceso).
"""
...
def _html_base(content_html: str, accent_color: str = VFL_BLUE) -> str:
    ...
    return (
        ...
        '<tr><td width="40" height="40" bgcolor="#0c2a45" style="border-radius:50%;text-align:center;vertical-align:middle">'
        ...
        '</td></tr></table><table></td></div></body></html>'
    )
...
def _order_card(title: str, rows_html: str) -> str:
    return f'<table width="100%" style="border:1.5px solid #e2e8f0;border-radius:12px">' \
           f'<td><td bgcolor="#f8fafc" style="padding:18px 20px;border-radius:12px">' \
...
def _steps_table(steps: list, accent: str = VFL_BLUE) -> str:
    ...
    for i, (title, desc) in enumerate(steps, 1):
        rows += f'<tr><td width="28" valign="top" style="padding:0 12px 12px 0"><table><td bgcolor="{accent}" width="28" height="28" style="border-radius:50%;text-align:center">' \
                f'<span style="color:#fff;font-size:11px;line-height:28px">{i}</span></td></td>' \
...
# ─────────────────────────────────────────────────────────────────────────────
# VOICE (TwiML) – SOLO UNA DEFINICIÓN (con entrada para "processing")
# ─────────────────────────────────────────────────────────────────────────────
...
async def build_premium_message_async(
    ...
    if event == "payment_request" and payment_url:
        # Asegurar que el enlace apunte a la cuenta del cliente
        base_url = BUSINESS_WEBSITE.rstrip('/')
        final_payment_url = f"{base_url}/account?pay_order={order_number}"
        logger.info(f"Payment request: redirecting to customer account {final_payment_url}")
